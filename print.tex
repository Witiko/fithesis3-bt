...
\thesislong{abstract}{This bachelor's thesis aims to assess the
  ...
  the findings, the author designs of a new \LaTeX\ document class
...
analyze the trends in the use of \gls{tex} across the faculties of
\gls{mu} and to assess the existing solutions for the typesetting
of theses. In this chapter, the author is going to summarize the
...
%  \end{table}
  ...
  As a part of the initial research, the author also assessed
  several \gls{tex} templates for the typesetting of theses in
  order to gather ideas for the design of the \textsf{fithesis3}
  \gls{docclass}.  Initially, the author is going to focus on
  templates used at Czech technical universities. At the end of the
  section, select generic thesis typesetting \glspl{docclass} will
  also be considered.

  \subsection{\textsf{CTUStyle}\index{\textsf{CTUStyle}} and
    \textsf{CUStyle}\index{\textsf{CUStyle}}}\label{sec:ctu&custyle}
  The first of the reviewed thesis typesetting templates were
  \textsf{CTUStyle} \cite{ctustyle} and \textsf{CUStyle}
  \cite{custyle} \glspl{texpackage} from RNDr. Petr Olšák, which
  were designed to be used at \gls{fel} and at \gls{cuni},
  ...
  \subsection{\textsf{Felthesis}\index{\textsf{felthesis}}}
    \label{sec:felthesis}
  ...
  \subsection{Technical University of
    Ostrava}\index{\textsf{diploma}|(}
  ...
  \subsection{Palacký Univerzity Olomouc}\label{sec:upol}
  ...
  \textsf{article} class, which the students are exprected to edit
  ...
  \subsection{Masaryk University in Brno}\label{sec:muni-styles}
...
describe the main design goals of \textsf{fithesis3}, the rationale
behind them and their implementation.
  ...
  \ref{sec:muni-styles}) in order to faciliate the adoption of
  ...
  spanning links, bibliographical citations, tables and the
  ...
  \begin{figure}
    \includegraphics[clip,trim=4.25cm 19.2cm 4.25cm 5.8cm]%
      {misc/metaveps.pdf}
    \caption{The comparison of the \gls{metafont} and vector
  ...
  In case of interest, the description of the complete public and
  private API of the class is available as a part of the technical
  documentation \cite{novotny15}, which is was submitted along with
  the thesis. Also submitted was the user documentation of the style
  files of \gls{mu}.

  \chapter{Conclusion}
  ...

  Being a part of the Dean's program for the support of software
  development projects, the development of the \textsf{fithesis3}
  document class will continue until the end of June 2015. During
  July--September 2015, the user guide will be expanded to its
  full length reflecting the user feedback submitted to the
  discussion forum \cite{fithesisForum}. An author's cookbook
  summarizing the formal requirements for theses will also be
  created and, drawing inspiration from the Department of
  Experimental Physics and the Department of Informatics at
  \gls{upol} (see Section \ref{sec:upol}), so will a \gls{beamer}
  template for thesis defense presentations and a template for
  the typesetting of reader's reports by thesis advisors and
  opponents.
