...
\thesislong{abstract}{This bachelor's thesis aims to assess the
  existing templates for the typesetting of theses and to analyze
  the trends in the usage of \gls{tex} at \acrlong{mu}. Based on
  the findings, the author designs a new \LaTeX\ document class
  for the typesetting of theses at \acrlong{mu}.}
...
% We redefine the title page
\makeatletter\thesis@load\makeatletter
\def\thesis@blocks@titlePage@content{%
  ...
  {\thesis@titlePage@Large\bf\thesis@author}\vfill
  This thesis was prepared for the SVOČ competition.}
\makeatother
...
writing a thesis in a run-of-the-mill word processor is not
unsurmountable, but it can be greatly simplified by the use of the
right tools. One of such tools is the \acrshort{latex} document
preparation system, which has been the industry standard for the
creation of scientific documents ever since its inception in 1984.
...
of \textsf{fithesis1} dating back to 2006, an updated version was
required. The main goal of this thesis was therefore to create a
new \gls{latex} document class that would fix the ailments of
...
author designed a new \gls{latex} thesis typesetting template. This
procedure is described in detail in Chapter \ref{sec:des&impl}.
The appendices \ref{sec:figuide}--\ref{sec:fspsguide} contain the
user documentation of the class for the various faculties of
\gls{mu} and Appendix \ref{sec:techdoc} contains the full technical
...
research.
  ...
  reason for the distortion of percentage.
  % The raw data obtained
  % from the questionnaire in Czech is available in the
  % \texttt{survey/survey.csv} file distributed alongside this
  % thesis.
...
universities. At the end of the section, select generic thesis
typesetting \glspl{docclass} will also be considered.

  \section{Charles University and Czech Technical~University}
  \subsection{\textsf{CUStyle}\index{\textsf{CUStyle}} and
    \textsf{CTUStyle}\index{\textsf{CTUStyle}}}\label{sec:ctu&custyle}
  ...
  which were designed to be used at \gls{fel} and at \gls{cuni},
  respectively. The sole dependencies of these templates are the
  \inx{\gls{csplain}} and \inx{\gls{opmac}} \glspl{texpackage}.
  ...
  \cite[section~3.2.3]{bpdpfi}. According the the statements of the
  ...
  \subsection{Official template of Czech Technical University}
    \label{sec:felthesis}
  The \textsf{felthesis} \gls{docclass} from Ing.\ Vít Zýka
  ...
  \textsf{fithesis2}, felthesis loads the KOMA-Script
  \textsf{scrreprt} \gls{docclass}, redefines some of the commands
  and \glspl{env}, loads additional packages and defines additional
  thesis sectioning commands. Unlike \textsf{fithesis1} and
  \textsf{fithesis2}, felthesis loads babel and then selects the
  language based on the chosen language of the thesis
  \cite[lines~687--691]{felthesisCode}. Felthesis also loads Bib%
  \LaTeX~\cite[line~722]{felthesisCode}, automatically generates
  index~\cite[line~763]{felthesisCode} and stamps the title,
  author, subject, and keywords into the header of the resulting
  \gls{pdf} file~\cite[lines~959--971]{felthesisCode}%
  \label{pdf-stamping}. Alongside the package \gls{ltxpackage},
  both a technical \cite{felthesisCode} and a user documentation
  \cite{felthesis} is distributed.
  ...
  document using the base \textsf{report} \gls{docclass} for both
  Czech and English theses. The students are expected to modify the
  said document to suit their requirements. The template uses
  \gls{cslatex} rather than the babel \gls{ltxpackage}, which means
  that \gls{csfonts} rather than the superior \gls{lmfonts}~%
  \cite{cslatexvsbabel} are used by default.
  ...
  Unlike the preceding typesetting templates, \textsf{tulthesis}
  from Doc.\ RNDr.\ Pavel Satrapa,\ Ph.D.\ is not a standalone
  \gls{docclass}, but rather a part of the tul \gls{ltxpackage},
  which is the official \gls{ltxpackage} used for the typesetting
  of documents on \gls{tul}. \textsf{Tulthesis} uses the base
  \textsf{report} \gls{docclass} \cite[section~1]{tulthesis-man}
  and it is quite visually distinctive. An interesting bit of
  trivia is the fact that it inspired
  \cite[post~21]{ctustyle-forums} the creation of Petr Olšák's
  \nameref{sec:ctu&custyle} \glspl{texpackage}. The tul
  \gls{ltxpackage}, along with the tulthesis \gls{docclass}, can be
  downloaded from \cite{tul}. \index{\textsf{tul}|)}
  \index{\textsf{tulthesis}|)}
  ...
  Hanzal from \gls{opf}, whose author describes the process of
  creating a \gls{gls-docclass} for what is supposed to be the
  official thesis typesetting template at \gls{opf}. There is,
  however, no mention of the style at the website of the faculty
  ...
  just en example \gls{latex} document using the base
  \textsf{article} class, which the students are expected to edit
  according to the needs of their theses. The template is supposed
  to be typeset using \glslink{cslatex}{\pdfCSLaTeX{}} \gls{texeng}
  meaning that \gls{csfonts} instead of the superior \gls{lmfonts}
  are used. Along with the thesis template, a \gls{beamer} template
  for thesis defense presentations is also distributed.
  Both templates can be downloaded at \cite{upol-exfyz-template}.
  ...
  designed as a general \gls{ltxpackage} for the typesetting of
  documents in the visual style of the Department of Informatics.
  Amongst the vices of the template are the disabled double-sided
  ...
  \gls{lmfonts} is, according to the author, too generous
  \cite[lines~74--82]{upol-inf-templateSource} and caused causing
  ...
  comprise thesis sectioning commands and \glspl{env} for the
  typesetting of mathematical definitions, theorems or proofs as
  well as code snippets. Along with the thesis template,
  \gls{latex} templates for the typesetting of thesis reader's
  reports and \gls{biblatex} ISO\hyph{}690~\cite{iso690-1,iso690-2}
  citation styles\pending{} are distributed. The templates can be
  downloaded at \cite{upol-inf-template}.\index{\textsf{KIdiplom}|)}
  ...
  lines as discussed earlier (see section \ref{overlong-lines}).
  ...
  manage to avoid overlong lines (see Section
  ...
  Section \ref{overlong-lines}). The \gls{ltxpackage} can be
  downloaded at \cite{graduate-thesis}.

  % TODO: Conclusion, move to a separate chapter
...
during December 2014--January 2015, the author developed the new
\textsf{fithesis3} \gls{docclass} during January--March 2015.  This
\gls{docclass} forms the implementation part of this thesis and in
this chapter, the author is going to describe the main goals of
\textsf{fithesis3}, its design and implementation.
  ...
  \index{felthesis}\hyperref[sec:felthesis]{\textsf{felthesis}}
  ...
  For these reasons, the author had decided to decompose the source
  code of the class into the following parts: \begin{enumerate}
  ...
  header of the output document, the corresponding locale and style
  files would be loaded and the mandatory parts specified by the
  style file would be appended to the document.
  ...
  locale for a given faculty at a given university are named
  ...
  For documentation and maintenance purposes, each locale
  consists of only one \texttt{\textit{locale}.dtx} \gls{dtx}
  file stored within the \texttt{locale/} directory. This file is
  subsequently decomposed into individual files using the
  ...
  \ref{fig:dirs-style}. The style files of a given faculty and a
  ...
  accordance with a Dean's directive or formal requirements
  published at the website of the given faculty \cite{ffdirective,
  ...
  mention, since it was deliberately designed to resemble
  the thesis typesetting template of \gls{sci} (see Section
  ...
  \Hologo{XeTeX} and \Hologo{LuaTeX} \glspl{texeng} is
  implemented by the \texttt{fithesis3-base.sty} style file stored
  within the \texttt{mu/} directory, which loads
  \gls{texeng}-specific packages based on the \gls{texeng} used.

  Drawing inspiration from Petr Olšák's
  \hyperref[sec:ctu&custyle]{\textsf{CTUStyle}
  \index{\textsf{CTUStyle}} and
  \textsf{CUStyle}\index{\textsf{CUStyle}} \glspl{texpackage}} (see
  Section \ref{sec:ctu&custyle}), the addition of support for
  colorful typesetting was added to the style files of \gls{mu}
  spanning links, %bibliographical citations, %TODO
  tables and the university and faculty logos.

  For documentation and maintenance purposes, the style file of each
  faculty at a given university consists of
  one \texttt{\textit{faculty}.dtx} \gls{dtx} file stored within
  the \texttt{\textit{university}/} subdirectory. This file is
  subsequently converted into the
  \texttt{fithesis3-\textit{faculty}.sty} file using the
  \texttt{\textit{faculty}.ins} \gls{ins} file and
  the \gls{makefile} stored within the \texttt{locale/} directory.
  ...
  \textit{university} is stored in the \texttt{base.pdf}
  \gls{pdf} and \texttt{base.eps} \gls{eps} files within the
  \texttt{\textit{university}/}
  subdirectory. The faculty logo of a given faculty at a
  given university is stored in the \texttt{\textit{faculty}}
  \gls{pdf} and \gls{eps} files within the
  \texttt{\textit{university}/} subdirectory. The
  \texttt{\textit{university}/color/}
  subdirectory is required by the style files
  of \gls{mu} when, color print is enabled and contains
  the color versions of logos of all faculties at \gls{mu}.
  ...
  \textsf{epstopdf} utility. The only exception to this was the
  ...
  \begin{figure}
    \includegraphics[clip,trim=4.25cm 19.2cm 4.25cm 5.8cm]%
      {misc/metaveps.pdf}
    \caption{The comparison of the \gls{metafont} and the vector
  ...
  written from a scratch. Although most of the class file concerns
  the processing of the input metadata, there are some parts that
  ...
  Section \ref{sec:felthesis}). The \textsf{fithesis3}
  automatically detects the main language of either the
  \textsf{babel} or the \textsf{polyglossia} package and sets it as
  the locale, when the locale is unspecified by the user.
  ...
  accessibility, has no grounds and should therefore be avoided.
  ...
  directory of the class. This file is subsequently converted into
  the \texttt{fithesis3.cls} file using the
  \texttt{\textit{locale}.ins} \gls{ins} file and the
  ...
  directory of the class as well as the user documentation of the
  \gls{docclass} into the \texttt{\textit{faculty}.pdf} files
  stored within the \texttt{guide/\textit{university}/}
  subdirectory.

  \section{Further reading}
  For an introduction to the class, consult the user documentation
  in appendices \ref{sec:figuide}--\ref{sec:fspsguide}. For the
  description of the complete public and private interface of the
  \gls{docclass}, see the technical documentation in Appendix
  \ref{sec:techdoc}. 

\chapter{Conclusion}
In the thesis, the author described the state of the \gls{latex}
thesis typesetting templates at \gls{fi} and the related software
projects prior to the author's involvement and the author's
contributions to help to bring the project to a successful closure.
...
results of this research will also be published as an article in
the \CS TUG bulletin \cite{cstug}.

Finally, the author designed and implemented a \textsf{fithesis3}
\gls{latex} document class, which will be used for the typesetting
of theses at the faculties of \gls{mu}. Being a part of the Dean's
program for the support of software development projects, the
testing of the \textsf{fithesis3} document class will continue
until the end of June 2015, when it will be published at online
collaboration platforms, \gls{ctan} and at the website of \gls{fi}.

During July--September 2015, the user guide will be
expanded to its full length reflecting the user feedback submitted
to the discussion forum \cite{fithesisForum}. An author's cookbook
...
reader's reports by thesis advisors and opponents.
